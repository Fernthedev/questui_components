...
namespace QUC {
    ...
    class WeakPtrHolder {
    public:
        using ComponentMap = std::unordered_map<QUC::inner::WeakPtrGOComponent const*, std::unordered_set<void*>>;

        // keepAliveGO can be gc'ed and this should still be safe
        // we are hashing the address
        static bool isPointerAlive(QUC::inner::WeakPtrGOComponent const* keepAliveGO, void* ptr) {
    ...
    private:
        ...
        friend class QUC::inner::WeakPtrGOComponent;
    };
    ...
    class WeakPtrGO {
    public:
        constexpr WeakPtrGO() = default; // assign ptr fields null
        ...
        void emplace(T* ptr) {
            ...
            auto* comp = go->GetComponent<QUC::inner::WeakPtrGOComponent *>();

            if (!comp) {
                comp = go->AddComponent<QUC::inner::WeakPtrGOComponent *>();
            }
